...
const Index=(props)=>{
            ...
            {props.length < 1? <tr key={0}><td colSpan={5}>......No data to display</td></tr>:(
                props.map((value, index) => {
                    return (
                        <tr key={index}>
                            <td>{index + 1}</td>
                            <td><Link to={`/dashboard/terms/Edit/${value.Id}`}>{value.Name}</Link></td>
    ...
    const search=async(query)=>{
        ...
        let sorted = terms.filter(function(item){
    ...
    const handleDelete=(id)=>{
        props.history.push(`/dashboard/terms/delete/${id}`)
    }
    const xtics ={
        Title: {Name: "Terms", Description: "Terms are the basic distinctions between sections of teachings."},
        Headers: headers,
        TableBody: BuildBody(tableSource),
        ItemCount: tableSource.length,
        Name: "Term",
        NewLink: "/dashboard/terms/New",
        Search: search
    }
