...
const Index=(props)=>{
    ...
    const headers = ["Name", "Last Updated", "Updater"]
     ...
     useEffect(() => {
        let getUrl = "subjects/getsubjects/";
            ...
            {props.length < 1? <tr key={0}><td colSpan={5}>......No data to display</td></tr>:(
                props.map((value, index) => {
                    return (
                        <tr key={index}>
                            <td>{index + 1}</td>
                            <td><Link to={`/dashboard/Subjects/Edit/${value.Id}`}>{value.Name}</Link></td>
    ...
    const search=async(query)=>{
        ...
        let sorted = subjects.filter(function(item){
    ...
    const handleDelete=(id)=>{
        props.history.push(`/dashboard/subjects/delete/${id}`)
    }
    const xtics ={
        Title: {Name: "School Subjects", Description: "List of subjects taught in the school."},
        Headers: headers,
        TableBody: BuildBody(tableSource),
        ItemCount: tableSource.length,
        Name: "Subject",
        NewLink: "/dashboard/Subjects/New",
        Search: search
    }
